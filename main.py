...
class ServiceHandler(http.server.SimpleHTTPRequestHandler):
    ...
    rel_path_db = "db/tangelo.sqlite"
    script_dir_db = os.path.dirname(__file__)
    abs_file_path_db = os.path.join(script_dir_db, rel_path_db)
    

    # GET Method Defination
    def do_GET(self):
        """ Hola Este es el doc de la prueba  	"""
            ...
            try:
                ...
                for region in reg_data:
                    ...
                    response_by_region = requests.get(
                        self.url_countries_by_region.format(region=region),
                    ...
                    while valid is False:
                        ...
                        if "languages" in list(
                            response_by_region[country_option].keys()
                        ):
                            valid = True
                        else:
                    ...
                    countries.append(
                        response_by_region[country_option]["name"]["common"]
                    )
                    key_language = list(
                        response_by_region[country_option]["languages"].keys()
                    )[0]
                    hash_languages.append(
                        hashlib.sha1(
                            response_by_region[country_option]["languages"][
                                str(key_language)
                            ].encode()
                        ).hexdigest()
                    )
    ...
    def insert_data(self, statistics) -> None:
        query_insert = f"""
